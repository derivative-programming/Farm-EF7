...
{
    internal class DFMaintenanceProviderConfiguration
    {
        ...
        public static DFMaintenanceProviderConfiguration GetConfig()
        {
            string sectionName = "FS.Farm" + "." + "DFMaintenanceProvider";
            DFMaintenanceProviderConfiguration result = null;
            if(ConfigurationManager.GetSection(sectionName) != null)
                result = (DFMaintenanceProviderConfiguration)ConfigurationManager.GetSection(sectionName);
            ...
            if (result == null && System.IO.File.Exists(overrideConfig))
            {
                ...
                if (systemConfiguration.GetSection(sectionName) != null)
                {
                    ...
                    IConfigurationSectionHandler sectionHandler = new DFMaintenanceProviderConfigurationHandler();
                    var xmlDocument = new XmlDocument();
                    xmlDocument.LoadXml(xml);
                    result = (DFMaintenanceProviderConfiguration)sectionHandler.Create(null, null, xmlDocument.DocumentElement);
                }
            }
            return result;
        }
