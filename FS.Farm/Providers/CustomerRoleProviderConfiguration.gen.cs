...
{
    internal class CustomerRoleProviderConfiguration
    {
        ...
        public static CustomerRoleProviderConfiguration GetConfig()
        {
            string sectionName = "FS.Farm" + "." + "CustomerRoleProvider";
            CustomerRoleProviderConfiguration result = null;
            if(ConfigurationManager.GetSection(sectionName) != null)
                result = (CustomerRoleProviderConfiguration)ConfigurationManager.GetSection(sectionName);
            ...
            if (result == null && System.IO.File.Exists(overrideConfig))
            {
                ...
                if (systemConfiguration.GetSection(sectionName) != null)
                {
                    ...
                    IConfigurationSectionHandler sectionHandler = new CustomerRoleProviderConfigurationHandler();
                    var xmlDocument = new XmlDocument();
                    xmlDocument.LoadXml(xml);
                    result = (CustomerRoleProviderConfiguration)sectionHandler.Create(null, null, xmlDocument.DocumentElement);
                }
            }
            return result;
        }
