...
{
    internal class FlavorProviderConfiguration
    {
        ...
        public static FlavorProviderConfiguration GetConfig()
        {
            string sectionName = "FS.Farm" + "." + "FlavorProvider";
            FlavorProviderConfiguration result = null;
            if(ConfigurationManager.GetSection(sectionName) != null)
                result = (FlavorProviderConfiguration)ConfigurationManager.GetSection(sectionName);
            ...
            if (result == null && System.IO.File.Exists(overrideConfig))
            {
                ...
                if (systemConfiguration.GetSection(sectionName) != null)
                {
                    ...
                    IConfigurationSectionHandler sectionHandler = new FlavorProviderConfigurationHandler();
                    var xmlDocument = new XmlDocument();
                    xmlDocument.LoadXml(xml);
                    result = (FlavorProviderConfiguration)sectionHandler.Create(null, null, xmlDocument.DocumentElement);
                }
            }
            return result;
        }
