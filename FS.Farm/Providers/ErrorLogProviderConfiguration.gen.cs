...
{
    internal class ErrorLogProviderConfiguration
    {
        ...
        public static ErrorLogProviderConfiguration GetConfig()
        {
            string sectionName = "FS.Farm" + "." + "ErrorLogProvider";
            ErrorLogProviderConfiguration result = null;
            if(ConfigurationManager.GetSection(sectionName) != null)
                result = (ErrorLogProviderConfiguration)ConfigurationManager.GetSection(sectionName);
            ...
            if (result == null && System.IO.File.Exists(overrideConfig))
            {
                ...
                if (systemConfiguration.GetSection(sectionName) != null)
                {
                    ...
                    IConfigurationSectionHandler sectionHandler = new ErrorLogProviderConfigurationHandler();
                    var xmlDocument = new XmlDocument();
                    xmlDocument.LoadXml(xml);
                    result = (ErrorLogProviderConfiguration)sectionHandler.Create(null, null, xmlDocument.DocumentElement);
                }
            }
            return result;
        }
