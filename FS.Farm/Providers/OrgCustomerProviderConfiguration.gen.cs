...
{
    internal class OrgCustomerProviderConfiguration
    {
        ...
        public static OrgCustomerProviderConfiguration GetConfig()
        {
            string sectionName = "FS.Farm" + "." + "OrgCustomerProvider";
            OrgCustomerProviderConfiguration result = null;
            if(ConfigurationManager.GetSection(sectionName) != null)
                result = (OrgCustomerProviderConfiguration)ConfigurationManager.GetSection(sectionName);
            ...
            if (result == null && System.IO.File.Exists(overrideConfig))
            {
                ...
                if (systemConfiguration.GetSection(sectionName) != null)
                {
                    ...
                    IConfigurationSectionHandler sectionHandler = new OrgCustomerProviderConfigurationHandler();
                    var xmlDocument = new XmlDocument();
                    xmlDocument.LoadXml(xml);
                    result = (OrgCustomerProviderConfiguration)sectionHandler.Create(null, null, xmlDocument.DocumentElement);
                }
            }
            return result;
        }
