...
{
    internal class OrganizationProviderConfiguration
    {
        ...
        public static OrganizationProviderConfiguration GetConfig()
        {
            string sectionName = "FS.Farm" + "." + "OrganizationProvider";
            OrganizationProviderConfiguration result = null;
            if(ConfigurationManager.GetSection(sectionName) != null)
                result = (OrganizationProviderConfiguration)ConfigurationManager.GetSection(sectionName);
            ...
            if (result == null && System.IO.File.Exists(overrideConfig))
            {
                ...
                if (systemConfiguration.GetSection(sectionName) != null)
                {
                    ...
                    IConfigurationSectionHandler sectionHandler = new OrganizationProviderConfigurationHandler();
                    var xmlDocument = new XmlDocument();
                    xmlDocument.LoadXml(xml);
                    result = (OrganizationProviderConfiguration)sectionHandler.Create(null, null, xmlDocument.DocumentElement);
                }
            }
            return result;
        }
