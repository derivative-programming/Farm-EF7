...
{
    internal class CustomerProviderConfiguration
    {
        ...
        public static CustomerProviderConfiguration GetConfig()
        {
            string sectionName = "FS.Farm" + "." + "CustomerProvider";
            CustomerProviderConfiguration result = null;
            if(ConfigurationManager.GetSection(sectionName) != null)
                result = (CustomerProviderConfiguration)ConfigurationManager.GetSection(sectionName);
            ...
            if (result == null && System.IO.File.Exists(overrideConfig))
            {
                ...
                if (systemConfiguration.GetSection(sectionName) != null)
                {
                    ...
                    IConfigurationSectionHandler sectionHandler = new CustomerProviderConfigurationHandler();
                    var xmlDocument = new XmlDocument();
                    xmlDocument.LoadXml(xml);
                    result = (CustomerProviderConfiguration)sectionHandler.Create(null, null, xmlDocument.DocumentElement);
                }
            }
            return result;
        }
