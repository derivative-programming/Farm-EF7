...
{
    internal class TriStateFilterProviderConfiguration
    {
        ...
        public static TriStateFilterProviderConfiguration GetConfig()
        {
            string sectionName = "FS.Farm" + "." + "TriStateFilterProvider";
            TriStateFilterProviderConfiguration result = null;
            if(ConfigurationManager.GetSection(sectionName) != null)
                result = (TriStateFilterProviderConfiguration)ConfigurationManager.GetSection(sectionName);
            ...
            if (result == null && System.IO.File.Exists(overrideConfig))
            {
                ...
                if (systemConfiguration.GetSection(sectionName) != null)
                {
                    ...
                    IConfigurationSectionHandler sectionHandler = new TriStateFilterProviderConfigurationHandler();
                    var xmlDocument = new XmlDocument();
                    xmlDocument.LoadXml(xml);
                    result = (TriStateFilterProviderConfiguration)sectionHandler.Create(null, null, xmlDocument.DocumentElement);
                }
            }
            return result;
        }
