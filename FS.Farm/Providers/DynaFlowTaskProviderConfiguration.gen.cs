...
{
    internal class DynaFlowTaskProviderConfiguration
    {
        ...
        public static DynaFlowTaskProviderConfiguration GetConfig()
        {
            string sectionName = "FS.Farm" + "." + "DynaFlowTaskProvider";
            DynaFlowTaskProviderConfiguration result = null;
            if(ConfigurationManager.GetSection(sectionName) != null)
                result = (DynaFlowTaskProviderConfiguration)ConfigurationManager.GetSection(sectionName);
            ...
            if (result == null && System.IO.File.Exists(overrideConfig))
            {
                ...
                if (systemConfiguration.GetSection(sectionName) != null)
                {
                    ...
                    IConfigurationSectionHandler sectionHandler = new DynaFlowTaskProviderConfigurationHandler();
                    var xmlDocument = new XmlDocument();
                    xmlDocument.LoadXml(xml);
                    result = (DynaFlowTaskProviderConfiguration)sectionHandler.Create(null, null, xmlDocument.DocumentElement);
                }
            }
            return result;
        }
