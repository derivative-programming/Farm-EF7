...
{
    internal class DFTDependencyProviderConfiguration
    {
        ...
        public static DFTDependencyProviderConfiguration GetConfig()
        {
            string sectionName = "FS.Farm" + "." + "DFTDependencyProvider";
            DFTDependencyProviderConfiguration result = null;
            if(ConfigurationManager.GetSection(sectionName) != null)
                result = (DFTDependencyProviderConfiguration)ConfigurationManager.GetSection(sectionName);
            ...
            if (result == null && System.IO.File.Exists(overrideConfig))
            {
                ...
                if (systemConfiguration.GetSection(sectionName) != null)
                {
                    ...
                    IConfigurationSectionHandler sectionHandler = new DFTDependencyProviderConfigurationHandler();
                    var xmlDocument = new XmlDocument();
                    xmlDocument.LoadXml(xml);
                    result = (DFTDependencyProviderConfiguration)sectionHandler.Create(null, null, xmlDocument.DocumentElement);
                }
            }
            return result;
        }
