...
{
    internal class LandProviderConfiguration
    {
        ...
        public static LandProviderConfiguration GetConfig()
        {
            string sectionName = "FS.Farm" + "." + "LandProvider";
            LandProviderConfiguration result = null;
            if(ConfigurationManager.GetSection(sectionName) != null)
                result = (LandProviderConfiguration)ConfigurationManager.GetSection(sectionName);
            ...
            if (result == null && System.IO.File.Exists(overrideConfig))
            {
                ...
                if (systemConfiguration.GetSection(sectionName) != null)
                {
                    ...
                    IConfigurationSectionHandler sectionHandler = new LandProviderConfigurationHandler();
                    var xmlDocument = new XmlDocument();
                    xmlDocument.LoadXml(xml);
                    result = (LandProviderConfiguration)sectionHandler.Create(null, null, xmlDocument.DocumentElement);
                }
            }
            return result;
        }
