    ...
    public partial class TacFarmDashboard
    {
        ...
        private static TacFarmDashboardDTO MapTacFarmDashboardDTO(QueryDTO data)
        {
            return new TacFarmDashboardDTO
            {
				...
				//TODO	cast((case when 1=0 then 1 else 0 end) as bit) as IsConditionalBtnAvailable.Value,

					TestFileDownloadLinkPacCode = data.pac.Code.Value,

					TestConditionalFileDownloadLinkPacCode = data.pac.Code.Value,

					TestAsyncFlowReqLinkPacCode = data.pac.Code.Value,

					TestConditionalAsyncFlowReqLinkPacCode = data.pac.Code.Value,
 ...
 *
	...
	(
		SELECT  Top  (@i_iPageNumber * @i_iItemCountPerPage)
			...
			Pac.Code as TestFileDownloadLinkPacCode,

			Pac.Code as TestConditionalFileDownloadLinkPacCode,

			Pac.Code as TestAsyncFlowReqLinkPacCode,
