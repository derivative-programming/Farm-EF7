    ...
    public class FlavorTest
    {
        ...
        public async Task AddAsync_NoExistingTransaction_ShouldAddFlavor()
        {
            ...
            using (var context = new FarmDbContext(options))
            {
                ...
                var flavor = await CreateTestFlavor(context);
        ...
        public async Task GetTotalCountAsync_WithFlavors_ShouldReturnCorrectCount()
        {
            ...
            using (var context = new FarmDbContext(options))
            {
                ...
                var manager = new FlavorManager(context);
                // Add some flavors
                await manager.AddAsync(await CreateTestFlavor(context));
                await manager.AddAsync(await CreateTestFlavor(context));
                await manager.AddAsync(await CreateTestFlavor(context));
                //// Add some flavors
                //context.FlavorSet.AddRange(
        ...
        public async Task GetMaxIdAsync_WithFlavors_ShouldReturnMaxId()
        {
            ...
            using (var context = new FarmDbContext(options))
            {
                ...
                var manager = new FlavorManager(context);
                // Add some flavors
                var flavor1 = await CreateTestFlavor(context);
                var flavor2 = await CreateTestFlavor(context);
                var flavor3 = await CreateTestFlavor(context);
                //context.FlavorSet.AddRange(flavor1, flavor2, flavor3);
                //await context.SaveChangesAsync();
        ...
        public async Task GetByIdAsync_ExistingFlavor_ShouldReturnCorrectFlavor()
        {
            ...
            using (var context = new FarmDbContext(options))
            {
                context.Database.EnsureCreated();
                var manager = new FlavorManager(context);
                var flavorToAdd = await CreateTestFlavor(context);
                await manager.AddAsync(flavorToAdd);
                //context.FlavorSet.Add(flavorToAdd);
                //await context.SaveChangesAsync();
        ...
        public async Task BulkInsertAsync_ValidFlavors_ShouldInsertAllFlavors()
        {
            ...
            using (var context = new FarmDbContext(options))
            {
                ...
                {
                    await CreateTestFlavor(context),
                    await CreateTestFlavor(context),
                    await CreateTestFlavor(context)
                };
        ...
        //ENDSET
        private async Task<Flavor> CreateTestFlavor(FarmDbContext dbContext)
        {
