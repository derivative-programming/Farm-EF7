    ...
    public class PacUserLandListTest
    {
        ...
        public async Task GetCountAsync_ShouldReturnSingleCount()
        {
            ...
            using (var context = new FarmDbContext(options))
            {
                ...
                Guid contextCode = GetPacCode(context, land1);
        ...
        private Guid GetPacCode(FarmDbContext dbContext, FS.Farm.EF.Models.Land land)
        {
            Dictionary<string,string> lineage = LandFactory.GetCodeLineage(dbContext, land.Code.Value);
