    ...
    public class PacUserTriStateFilterListTest
    {
        ...
        public async Task GetCountAsync_ShouldReturnSingleCount()
        {
            ...
            using (var context = new FarmDbContext(options))
            {
                ...
                Guid contextCode = GetPacCode(context, triStateFilter1);
        ...
        private Guid GetPacCode(FarmDbContext dbContext, FS.Farm.EF.Models.TriStateFilter triStateFilter)
        {
            Dictionary<string,string> lineage = TriStateFilterFactory.GetCodeLineage(dbContext, triStateFilter.Code.Value);
