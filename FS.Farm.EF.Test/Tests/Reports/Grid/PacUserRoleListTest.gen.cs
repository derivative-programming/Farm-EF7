    ...
    public class PacUserRoleListTest
    {
        ...
        public async Task GetCountAsync_ShouldReturnSingleCount()
        {
            ...
            using (var context = new FarmDbContext(options))
            {
                ...
                Guid contextCode = GetPacCode(context, role1);
        ...
        private Guid GetPacCode(FarmDbContext dbContext, FS.Farm.EF.Models.Role role)
        {
            Dictionary<string,string> lineage = RoleFactory.GetCodeLineage(dbContext, role.Code.Value);
